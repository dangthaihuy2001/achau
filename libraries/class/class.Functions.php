...
class Functions
{
	...
	public function check_permission_user()
	{
		...
		if ($_SESSION[$login_admin]['role'] == 3) {
			return 3;
		} elseif ($_SESSION[$login_admin]['role'] == 2) {
			return 2;
		} elseif ($_SESSION[$login_admin]['role'] == 1) {
			return 1;
		}
	}
	...
	/* Get tags */
	public function get_tags2($id = 0, $element = '', $table = '', $type = '', $typemain = '')
	{
		if ($id) {
			$temps = $this->d->rawQueryOne("select id_city from #_" . $table . " where id = ? and type = ? limit 0,1", array($id, $typemain));
			$arr_tags = explode(',', $temps['id_city']);

			for ($i = 0; $i < count($arr_tags); $i++) $temp[$i] = $arr_tags[$i];
		}

		$row_tags = $this->d->rawQuery("select ten, id from #_city where hienthi > 0 order by stt,id desc");
	...
	/* Get place by ajax */
	public function get_ajax_place($table = '', $title_select = 'Chọn danh mục', $id_city = 0, $id_wards = '')
	{
		$where = '';
		$params = array('0');
		$id_parent = 'id_' . $table;
		...
		if ($table == 'city') {
			...
			$id_city = (isset($_REQUEST['id_city'])) ? htmlspecialchars($_REQUEST['id_city']) : $id_city;
			$where .= ' and id_city = ?';
		...
		foreach ($rows as $v) {
			if (isset($_REQUEST[$id_parent]) && ($v["id"] == (int)$_REQUEST[$id_parent])) $selected = "selected";
			elseif ($id_city == $v["id"]) $selected = "selected";
			else $selected = "";
	...
	public function getTime($timestamp)
	{
		...
		// Tính khoảng cách thời gian giữa thời gian tạo và thời gian hiện tại
		$time_difference = $current_time - $timestamp;
		//var_dump($time_difference);die();
		// Kiểm tra nếu khoảng cách thời gian nhỏ hơn 1 ngày (24 giờ)
		if ($time_difference < 86400) {
			// Tính khoảng cách thời gian theo giờ
			$hours = floor($time_difference / 3600);
			if ($hours <= 0) {
				// Nếu dưới 1 giờ, trả về số phút
				$minutes = floor($time_difference / 60);
				return "$minutes phút trước";
			} else {
				// Nếu trên 1 giờ, trả về số giờ
				return "$hours giờ trước";
			}
		} else {
			// Nếu khoảng cách thời gian trên 1 ngày, tính khoảng cách theo ngày
			$days = floor($time_difference / 86400);
			return "$days ngày trước";
		}
	}

	public function getAdminCurrent($id)
	{
		$row = $this->d->rawQueryOne("select * from #_user where id = ? limit 1", array($id));
		return $row;
	}
	public function getUserCurrent($id)
	{
		$row = $this->d->rawQueryOne("select * from #_member where id = ? limit 1", array($id));
		return $row;
	}
	...
	public function checkLocation($id, $id_city = 0)
	{
		if ($id_city != 0) {
			...
			if ($_COOKIE['location'] == $id) {
				return 1;
			}
		}
		return 0;
	}
	public function getIdCityCurrent()
	{
		if (isset($_GET['id_city']) && $_GET['id_city'] > 0) {
			return $_GET['id_city'];
		} elseif (isset($_COOKIE['location']) && $_COOKIE['location'] > 0) {
