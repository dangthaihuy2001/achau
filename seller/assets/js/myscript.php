<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$(window).load(function() {
			<?php if (!isset($_GET['id_wards'])) { ?>
				var id = $('.select-place').val();
				var child = $('.select-place').data("child");
				var level = parseInt($('.select-place').data('level'));
				var table = $('.select-place').data('table');
				console.log(child, level, table)
				if ($("#" + child).length) {
					$.ajax({
						url: 'ajax/ajax_place.php',
