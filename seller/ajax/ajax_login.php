...
//	$username = (!empty(1)); $_SERVER['REMOTE_ADDR'] = 1; //")) ? $_POST['username'] : '';
...
if ($error == '') {
	...
	if ($username == '' && $password == '') {
		...
		/* Kiểm tra đăng nhập */
		$row = $d->rawQueryOne("select * from #_user where username = ? and hienthi>0 and role = 1 limit 0,1", array($username));

		if (isset($row['id'])) {
			$checkTime = $row['thoihan'] - time();
			if ($checkTime < 0) {
		...
		if (isset($row['id'])) {
			...
			} else if (($row['password'] == $func->encrypt_password($config['website']['secret'], $password, $config['website']['salt']) && $row['role'] == 1)) {
				$timenow = time();
				$id_user = $row['id'];
				$ip = $func->getRealIPAddress();
				$token = md5(time());
