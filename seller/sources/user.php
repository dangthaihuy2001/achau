...
switch ($act) {
	...
	/* Info admin banhang */
	case "man_admin_seller":
		/* Kiểm tra active user admin */
		if (!isset($config['user']['admin']) || $config['user']['admin'] == false) $func->transfer("Trang không tồn tại", "index.php", false);
		if ($func->check_permission_user() == 1) {
			$func->transfer("Bạn không có quyền vào trang này", "index.php", false);
			exit;
		}
		get_items_admin_seller();
		$template = "user/man_seller/items";
		break;
	case "add_admin_seller":
		/* Kiểm tra active user admin */
		if (!isset($config['user']['admin']) || $config['user']['admin'] == false) $func->transfer("Trang không tồn tại", "index.php", false);
		if ($func->check_permission_user() == 1) {
			$func->transfer("Bạn không có quyền vào trang này", "index.php", false);
		...
		$template = "user/man_seller/item_add";
		break;
	case "edit_admin_seller":
		/* Kiểm tra active user admin */
		if (!isset($config['user']['admin']) || $config['user']['admin'] == false) $func->transfer("Trang không tồn tại", "index.php", false);
		if ($func->check_permission_user() == 1) {
			$func->transfer("Bạn không có quyền vào trang này", "index.php", false);
		...
		get_item_admin_seller();
		$template = "user/man_seller/item_add";
		break;
	case "save_admin_seller":
		/* Kiểm tra active user admin */
		if (!isset($config['user']['admin']) || $config['user']['admin'] == false) $func->transfer("Trang không tồn tại", "index.php", false);
		save_item_admin_seller();
		break;
	case "delete_admin_seller":
		/* Kiểm tra active user admin */
		if (!isset($config['user']['admin']) || $config['user']['admin'] == false) $func->transfer("Trang không tồn tại", "index.php", false);
		if ($func->check_permission_user() == 1) {
			$func->transfer("Bạn không có quyền vào trang này", "index.php", false);
...
function get_items_admin()
{
	...
	$startpoint = ($curPage * $per_page) - $per_page;
	$limit = " limit " . $startpoint . "," . $per_page;
	$sql = "select * from #_user where id <> 116 and role = 2 $where order by stt,id desc $limit";
	$items = $d->rawQuery($sql);
	$sqlNum = "select count(*) as 'num' from #_user where id <> 116  and role = 2 $where order by stt,id desc";
	$count = $d->rawQueryOne($sqlNum);
	$total = $count['num'];
...
/* Get admin */
function get_items_admin_seller()
{
	global $d, $func, $curPage, $items, $paging, $config, $login_admin;
	...
	if (isset($_REQUEST['id_city'])) {
		$id = htmlspecialchars($_REQUEST['id_city']);
		$where .= " and id_city = " . $id;
	}
	//role admin_seller session
	if ($_SESSION[$login_admin]['role'] == 2) {
		$detailUser =  $func->getAdminCurrent($_SESSION[$login_admin]['id']);
		$where .= " and id_city = " . $detailUser['id_city'];
	}

	$per_page = 50;
	$startpoint = ($curPage * $per_page) - $per_page;
	$limit = " limit " . $startpoint . "," . $per_page;
	$sql = "select * from #_user where id <> 116 and role = 1 $where order by stt,id desc $limit";
	$items = $d->rawQuery($sql);
	$sqlNum = "select count(*) as 'num' from #_user where id <> 116  and role = 1 $where order by stt,id desc";
	$count = $d->rawQueryOne($sqlNum);
	$total = $count['num'];
	$url = "index.php?com=user&act=man_admin_seller";
	$paging = $func->pagination($total, $per_page, $curPage, $url);
}
/* Edit admin */
function get_item_admin_seller()
{
	global $d, $func, $curPage, $item;

	$id = (isset($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;

	if (!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=user&act=man_admin_seller&p=" . $curPage, false);

	$item = $d->rawQueryOne("select * from #_user where id = ? and id!= 1 limit 0,1", array($id));

	if (!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=user&act=man_admin_seller&p=" . $curPage, false);
}

/* Save admin */
function save_item_admin_seller()
{
	global $d, $func, $curPage, $config;

	if (empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=user&act=man_admin_seller&p=" . $curPage, false);
	...
	$username = isset($data['username']) ? $data['username'] : '';
	$check_username = $d->rawQueryOne("select id from #_user where username = ? and id <> ? limit 0,1", array($username, $id));
	if (!empty($check_username)) $func->transfer("Tên đăng nhập này đã tồn tại. Xin chọn tên khác", "index.php?com=user&act=man_admin_seller&p=" . $curPage, false);

	if ($id) {
		if ($func->check_permission_user() == 1) {
			$row = $d->rawQueryOne("select id from #_user where id = ? limit 0,1", array($id));
			if (isset($row['id']) && $row['id'] > 0) $func->transfer("Bạn không có quyền trên tài khoản này. Mọi thắc mắc xin liên hệ quản trị website", "index.php?com=user&act=man_admin_seller&p=" . $curPage, false);
		}
		...
		if (isset($data['password']) && ($data['password'] != '')) {
			...
			if ($confirm_password == '') {
				$func->transfer("Chưa xác nhận mật khẩu mới", "index.php?com=user&act=edit_admin_seller&id=" . $id . "&p=" . $curPage, false);
			}

			if ($password != $confirm_password) {
				$func->transfer("Xác nhận mật khẩu mới không chính xác", "index.php?com=user&act=edit_admin_seller&id=" . $id . "&p=" . $curPage, false);
			}
		...
		$d->where('id', $id);
		if ($d->update('user', $data)) {
			$func->transfer("Cập nhật dữ liệu thành công", "index.php?com=user&act=man_admin_seller&p=" . $curPage);
		} else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=user&act=man_admin_seller&p=" . $curPage, false);
	} else {
...
/* Delete admin */
function delete_item_admin_seller()
{
	global $d, $func, $curPage;
	...
	if ($id) {
		$d->rawQuery("delete from #_user where id = ? and id!= 1", array($id));
		$func->transfer("Xóa dữ liệu thành công", "index.php?com=user&act=man_admin_seller&p=" . $curPage);
	} elseif (isset($_GET['listid'])) {
		$listid = explode(",", $_GET['listid']);
	...
	$func->transfer("Không nhận được dữ liệu", "index.php?com=user&act=man_admin_seller&p=" . $curPage, false);
}
...
function get_items()
{
	...
	if (isset($_REQUEST['id_city'])) {
		$keyword = htmlspecialchars($_REQUEST['id_city']);
		$where .= " and id_city = " . $keyword;
	}
	if (isset($_REQUEST['id_wards'])) {
		$keyword = htmlspecialchars($_REQUEST['id_wards']);
		$where .= " and id_wards = " . $keyword;
	}
	//check phân quyền
	if ($_SESSION[$login_admin]['role'] == 2) {
		$detailUser =  $func->getAdminCurrent($_SESSION[$login_admin]['id']);
...
function edit()
{
	...
	if (!empty($_POST)) {
		...
		$row_detail_user = $d->rawQueryOne("select * from #_user where username = ? limit 1", array($_SESSION[$login_admin]['username']));
		if ($row_detail_user['username'] != $_POST['data']['username']) {
			$func->transfer("Không có quyền thực hiện thao tác này","index.php?com=user&act=admin_edit", false);
		} elseif ($row_detail_user['ten'] != $data['ten']) {
			$func->transfer("Không có quyền thực hiện thao tác này","index.php?com=user&act=admin_edit", false);
		} elseif ($row_detail_user['dienthoai'] != $data['dienthoai']) {
			$func->transfer("Không có quyền thực hiện thao tác này","index.php?com=user&act=admin_edit", false);
		} elseif ($row_detail_user['diachi'] != $data['diachi']) {
			$func->transfer("Không có quyền thực hiện thao tác này","index.php?com=user&act=admin_edit", false);
		} elseif ($row_detail_user['toado'] != $data['toado']) {
			$func->transfer("Không có quyền thực hiện thao tác này","index.php?com=user&act=admin_edit", false);
		} elseif ($row_detail_user['toado_iframe'] != $data['toado_iframe']) {
			$func->transfer("Không có quyền thực hiện thao tác này","index.php?com=user&act=admin_edit", false);
		}
