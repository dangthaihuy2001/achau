...
function save_item_admin_seller()
{
	...
	if ($id) {
		$row = $d->rawQueryOne("select * from #_user where id = ? limit 0,1", array($id));
		if (isset($row['id']) && $row['id'] > 0 && $row['role'] != 1) {
			$func->transfer("Bạn không có quyền trên tài khoản này. Mọi thắc mắc xin liên hệ quản trị website", "index.php?com=user&act=man_admin_seller&p=" . $curPage, false);
		} else {
			$getProductAllUser = $d->rawQuery("select id from #_product where id_nguoiban = ?", array($id));
			$dataProduct['id_city'] = $data['id_city'];
			$dataProduct['id_wards'] = $data['id_wards'];			
			foreach ($getProductAllUser as $key => $value) {
				$d->where('id', $value['id']);
				$d->update('product', $dataProduct);
			}
		}
