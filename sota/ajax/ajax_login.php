...
//	$username = (!empty(1)); $_SERVER['REMOTE_ADDR'] = 1; //")) ? $_POST['username'] : '';
	...
	if($error == '')
	{
		...
		else
		{
			...
			/* Kiểm tra đăng nhập */
			$row = $d->rawQueryOne("select * from #_user where username = ? and hienthi>0 and role = 3 limit 0,1",array($username));
			...
			if(isset($row['id'])){
				if((int)$_COOKIE['login_admin']==$row['id'] && $_COOKIE['login_session']==$row['login_session']){
					...
					$error = "Tài khoản này không có quyền đăng nhập admin Tổng";
				}
