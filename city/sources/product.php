...
function get_items()
{
	...
	if ($_SESSION[$login_admin]['id'] != 116) {
		...
		if ($_SESSION[$login_admin]['role'] == 2) {
			...
			$where .= " and id_city=" . $userCurrent['id_city'];
			...
			$where .= " and id_nguoiban=" . $userCurrent['id'];
			$where .= " and id_city=" . $userCurrent['id_city'];
		}
	}
...
function get_item()
{
	global $d, $func, $strUrl, $curPage, $item, $gallery, $type, $com, $act, $getRating, $login_admin;
	...
	if ($_SESSION[$login_admin]['id'] == 116 && $_SESSION[$login_admin]['role'] == 3) {
		...
		$item = $d->rawQueryOne("select * from #_product where id = ? and id_admin_city = ? and type = ? limit 0,1", array($id, $func->getAdminCurrent($_SESSION[$login_admin]['id'])['id_city'], $type));
	}
	...
	if ($data) {
		...
		if (isset($_SESSION[$login_admin]['active']) && $_SESSION[$login_admin]['active'] == true) {
			...
			if ($_SESSION[$login_admin]['role'] == 3) {
				$data['id_nguoiban'] = $_SESSION[$login_admin]['id'];
			} elseif ($_SESSION[$login_admin]['role'] == 2) { //admin tỉnh
				$data['id_nguoiban'] = $_SESSION[$login_admin]['id'];
				$id_admin_city = $d->rawQueryOne("select id, id_city from #_user where id = ? and role = 1 limit 1", array($_SESSION[$login_admin]['id']));
				$data['id_city'] = $id_admin_city['id_city'];
				$data['id_admin_city'] = $id_admin_city['id_city'];
			} elseif ($_SESSION[$login_admin]['role'] == 1) { //admin người bán
				$data['id_nguoiban'] = $_SESSION[$login_admin]['id'];
				$id_admin_city = $d->rawQueryOne("select id, id_city from #_user where id = ? and role = 1 limit 1", array($_SESSION[$login_admin]['id']));
				$data['id_city'] = $id_admin_city['id_city'];
...
function delete_item()
{
	...
	if ($id) {
			/* Lấy dữ liệu */
		$row = $d->rawQueryOne("select id, photo, id_admin_city from #_product where id = ? and type = ? limit 0,1", array($id, $type));

		if($row['id_admin_city'] != $func->getAdminCurrent($_SESSION[$login_admin]['id'])['id_city']){
			$func->transfer("Cảnh báo: không có quyền thực hiện tao tác này", "index.php?com=product&act=man&type=" . $type . $strUrl, false); 
		}
...
function save_list()
{
	...
	if ($data) {
		...
		if (isset($config['product'][$type]['city_list']) && $config['product'][$type]['city_list'] == true) {
			if (isset($_POST['city_group']) && ($_POST['city_group'] != '')) $data['id_city'] = implode(",", $_POST['city_group']);
			else $data['id_city'] = "";
		}
	}
